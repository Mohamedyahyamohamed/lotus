...
# 
# # دالة استخراج الأرقام من PDF (Heuristic/Regex Based)
...
# 
#         # محاولة البحث عن أنماط بسيطة للأرقام (تجريبي)
#         # بندور على كلمة Mean وجنبها رقم، و SD وجنبها رقم
#         # ده حل "Best Effort" لأن كل PDF شكله مختلف
# 
#         # بحث عن حجم العينة (n=...)
...
# 
#             # لو معرفناش نطلع داتا، بنحط قيم افتراضية عشان الرسمة متقفش
#             if stats['N'] == 0:
...
#             st.download_button(
#                 label="📄 Download Screening List (CSV)",
#                 data=csv_screen,
...
#             )
# 
#     st.divider() # خط فاصل للشكل الجمالي
# 
...
#         if not st.session_state['stats'].empty:
#             st.write("✍️ **Edit Data Here (Double click cells):**")
# 
...
# 
#                     # تحديث الـ State عشان لما تحمل الملف ينزل بالأرقام الجديدة
...
try:
    ...
    print(f"🚀 الرابط جاهز وشغال يا بطل: {public_url}")
except Exception as e:
    print(f"Error: {e}")
...
# 1. تحديد مكان ngrok بدقة (عشان نمنع خطأ FileNotFound)
# أمر apt install عادة بيحط البرنامج في /usr/bin/ngrok
ngrok_path = "/usr/bin/ngrok"

# لو ملقهوش هناك، نجرب المسار التاني المحتمل
if not os.path.exists(ngrok_path):
    ngrok_path = "/usr/local/bin/ngrok"

# إجبار المكتبة تستخدم المسار ده
...
# 4. تشغيل التطبيق في الخلفية
# (تأكد إن ملف app.py موجود جنبك)
os.system("nohup streamlit run app.py --server.port 8501 >/dev/null 2>&1 &")

# انتظار بسيط للتأكد من تشغيل السيرفر
time.sleep(3)

# 5. فتح النفق
try:
    public_url = ngrok.connect(8501).public_url
    print(f"🚀 الرابط جاهز يا دكتور: {public_url}")
except Exception as e:
    print(f"Error: {e}")
